...
<main class="gap-5 md:flex">
  ...
  <section class="text-tertiary grid grid-cols-auto350 gap-5 items-start ">
    ...
    <article class="bg-secondary p-4 rounded-xl relative" x-data="{ toggleDone: false }">
      <header class="flex justify-end  ">
        <div class="absolute top-2 right-4" x-data="{ open: false }">
          ...
          <div class="flex flex-col bg-primary text-light rounded-xl w-40 p-1 absolute top-5 right-0" x-show="open" x-transition x-cloak>
            <a href="" class="p-3 border-b border-solid border-secondary ">Edit...</a>
      ...
      <div class="flex justify-between ">
        ...
        <div class="">
          <a href='#1' id="1" class="flex gap-3 items-center" x-on:click="toggleDone = ! toggleDone">
...
<section class="fixed top-0 left-0 bg-black w-full h-screen flex justify-center items-center bg-opacity-40">
  <form action="{{ route('todos.update', $editTodo) }}" method="POST" class="bg-tertiary text-primary p-7 rounded-2xl flex-initial w-[500px] text-sm ">
    @method('PUT')
    @csrf
    ...
    <div class="mb-7">
      ...
      <input type="text" class="block w-full p-2 rounded-lg bg-light" value="{{ $editTodo->title }}" name="title" id="todo-form-title" placeholder="Add a title">
    </div>

    <div class="mb-7">
      <label for="todo-form-desc" class="mb-3 block font-semibold text-xl">Description</label>
      <textarea rows="4" class="block w-full p-2 rounded-lg bg-light" name="description" id="todo-form-desc" placeholder="Add a description">{{ $editTodo->description }}</textarea>
    ...
    <div>
      ...
      <div class="text-base">
        @foreach ($categories as $category)
        <div class="">
          <input type="radio" name="category_id" value="{{ $category->id }}" id="category-{{  $category->id  }}" @checked($category->id == $editTodo->category_id)>
